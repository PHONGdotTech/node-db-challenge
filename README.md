...
A relational database is a type of database that uses a relationship model between entities, whereas SQL is just the language used to interact with databases.

- [ ] Why do tables need a `primary key`?

Tables need a primary key to uniquely identify each record from another record within that table.
...
In order to have a many to many relationship between two tables you need an associative table that consists of a composite primary key, which itself is made up of the combination of the primary keys of both tables having the relationship.
